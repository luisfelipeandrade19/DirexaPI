...
function GoogleButton() {
  ...
  const handleSuccess = async (credentialResponse: CredentialResponse) => {
    ...
    if (credentialResponse.credential) {
      try {
        const decoded: GoogleUser = jwtDecode<GoogleUser>(
        ...
        if (error) {
          console.error(
            "Erro no login com Supabase via Google:",
            error.message
          ); // Loga o erro
        ...
        if (data.user) {
          // Se o usuário foi autenticado/registrado no Supabase
            ...
            .upsert(
              {
                ...
                avatar_url:
                  data.user.user_metadata?.avatar_url || decoded.picture, // URL do avatar
                // Adicione outros campos que você deseja salvar
              },
              { onConflict: "id" }
            ); // Atualiza se o ID já existir

          if (profileError) {
            console.error(
              "Erro ao salvar/atualizar perfil do usuário:",
